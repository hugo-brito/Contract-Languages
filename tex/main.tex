...
Not all contracts can be implemented using a contract language as it requires the given contract to be unambiguous in nature, such as financial contracts and in general contracts governing the exchange of resources. On top pf this, some aspects of a contract that are simply too complex to model. % comment from patrick: Is that the only reason?
...
\section{What is Testing?} % Willard: an example would be nice
...
\subsection{Property-based Testing} % Willard: an example would be nice, e.g. quickcheck
...
% Comment from patrick: I would like to see the entirety of the evalC definition here, along with an explanation of the following: (1) what is the intended semantics of each combinator, and (2) how is this intended semantics reflected in the implementation of evalC.
